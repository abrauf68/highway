            ...
            <div class="row gy-4 d-flex justify-content-between">
                <div class="col-lg-6 order-2 order-lg-1 d-flex flex-column justify-content-center">
                    ...
                    <p data-aos="fade-up" data-aos-delay="100">
                        We specialize in secure, coast-to-coast vehicle transport — combining speed, precision, and
                        real-time updates. Whether it’s your daily driver or a prized collectible, our team ensures a
                        smooth, worry-free delivery from pickup to drop-off.
                    </p>
                                    ...
                                    <div class="col-12 mt-3">
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="condition"
                                                value="running" required checked>
                ...
                if (brandId) {
                    $.ajax({
                        ...
                        success: function(response) {
                            $('#vehicleModel').empty().append(
                                '<option value="" disabled selected>Select Model</option>');

                            if (response.success && response.models.length > 0) {
                                $.each(response.models, function(key, model) {
                                    $('#vehicleModel').append('<option value="' + model
                                        .name + '">' + model.name + '</option>');
                                });
                            }
                        },
                        error: function() {
            ...
            if (pickupZip && deliveryZip) {
                Promise.all([
                    ...
                    .then(([pickupData, deliveryData]) => {
                        const [lat1, lon1] = [parseFloat(pickupData.places[0].latitude), parseFloat(pickupData.places[0]
                            .longitude)];
                        const [lat2, lon2] = [parseFloat(deliveryData.places[0].latitude), parseFloat(deliveryData
                            .places[0].longitude)];
